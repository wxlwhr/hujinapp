...
<template>
  <el-card class="continue">
        ...
        <el-row>
          <el-col :span="12" class="textLight">
            <div class="upload">
              ...
              >
                <el-button type="primary">批量导入检测结果<i class="el-icon-upload el-icon--right" /></el-button>
              </el-upload>
              <el-button type="primary" style="marginLeft:15px" @click="riskDownload()">下载模板</el-button>
            </div>
          </el-col>
    ...
    />
    <lookResult v-if="dialogTips" :look-visible="dialogTips" :disable="disable" :dialog-msg="dialogMsg" @dialogHide="dialogHide" />
...
export default {
  ...
  computed: {
    actionUrl() {
      return `nifaReport/app/risk/InputExcel?username=${this.$store.state.user.userName}`
    },
    queryMessage() {
      return {
        pageNumber: 1,
  ...
  methods: {
    ...
    riskDownload() {
      location.href = `nifaReport/app/risk/download`
    },
    // 上传文件成功的回调

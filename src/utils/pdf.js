...
const upPdf = function queryByRinm(params) {
  return service({
    url: `nifa-autonomy-file/upload/finalSave`,
    method: 'post',
    data: params
  })
}
// ?&ownSystem=30&ownBusiness=nifaReport
/**
 * @param  ele          要生成 pdf 的DOM元素（容器）
 * @param  padfName     PDF文件生成后的文件名字
 * */

export function upLoadPDF(ele, pdfName) {
  // debugger
  window.scrollTo(0, 0)
  ...
  html2canvas(ele, {
    ...
    // 可动态生成
    // pdf.save(pdfName)下载功能
    const datauri = pdf.output('dataurlstring')
    const base64 = datauri.split('base64,')[1]
    const obj = {
      data: base64,
      noteId: store.getters.riid,

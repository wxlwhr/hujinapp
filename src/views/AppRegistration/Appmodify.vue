<template>
  <el-card class="AppRegistrationDetailVivw">
    ...
    <el-form ref="form" :model="form" :rules="rules">
      ...
      <fieldset v-if="($route.query.auditStatus === '审核未通过' || $route.query.auditStatus === '审核不通过') && $route.query.name !== '一般变更'" class="fieldsetStyle">
        <legend style="margin-left: 40px; font-size:17px">外部评估</legend>
        <el-row :gutter="20">
          <el-col v-if="$route.query.auditStatus !== '检测退至金融机构' && $route.query.auditStatus !== '认证退至金融机构' && $route.query.auditStatus !== '审核未通过'" :span="8">
          ...
          <el-col v-if="addApp" :span="8">
            <el-form-item label="检测机构" prop="orgId">
              ...
              <el-select v-model="form.detectionOrg" placeholder="请选择检测机构" clearable>
      ...
      <fieldset v-else class="fieldsetStyle">
        <legend style="margin-left: 40px; font-size:17px">外部评估</legend>
        <el-row v-if="$route.query.tips === '2'" :gutter="20">
          <el-col :span="12">
            <el-form-item label="认证机构" prop="approveOrg">
              <el-input v-show="false" v-model="form.approveOrg" placeholder="" />
              <el-tooltip class="item" effect="dark" content=" 选择该认证机构后，认证机构可查看申报机构的联系人信息" placement="top">
                <i class="el-icon-question " />
              </el-tooltip>
              <el-select
          ...
          <el-col :span="12">
            <el-form-item label="选择检测机构" prop="orgId">
              ...
              <el-select v-model="form.detectionOrg" :disabled="returnDisabled" placeholder="请选择检测机构" clearable>
                <el-option v-for="(item,index) in optionsTest" :key="index" :label="item.rinm" :value="item.riid" />
              </el-select>
            </el-form-item>
          </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        detectionOrg: [{ required: true, message: '请输入', trigger: 'blur' }],
        ...
        if (this.remnant === this.remnant2) {
          ...
          if (this.bc) {
            ...
            if (this.$route.query.num === '2' && this.$route.query.update !== '重大变更') {
              ...
              this.paramsObj.oldattachIds = this.oldattachIds.join(',')
              this.paramsObj.deleteattachIds = this.deleteattachIds
              if (this.paramsObj.appFilePath === '1') {
                ...
                this.paramsObj.flag = '3'// 跨级退回提交时flag是3，不跨级是2，4是金融直接到协会
                proChange(this.paramsObj, this.form.approveOrg).then(res => {
                  this.getStatus('提交审核成功')
                }).catch(() => {
                  this.loading.close()
              ...
              if (this.rowData.auditStatus === '协会退至金融机构') {
                if (this.form.approveInfo || this.$route.query.changeTips) {
                  this.paramsObj.flag = '4' // 4是直接到协会
              ...
              } else if (this.rowData.auditStatus === '检测退至金融机构') {
                if (this.form.approveInfo || this.$route.query.changeTips) {
  ...
  mounted() {
    console.log(6666, this.$route.query)
    this.riidNum2 = getUUIDWG()
    ApprecordDetails(this.$route.query.id).then(res => {
      this.rowData = res
      if (this.$route.query.name === '一般变更' || this.rowData.auditStatus === '协会退至金融机构') {
      ...
      if (this.form.approveInfo) {
        ...
        if (this.form.detectionInfo) {
          ...
          this.newOptionsTest.push(this.form.detectionInfo)
      ...
      if (this.rowData.appFilePath === '1') {
        this.addApp = false
        this.addVal = '其他'
      } else if (this.rowData.appFilePath === '2') {
        this.addApp = true
        this.addVal = '金融科技产品认证'
      }
      if (this.rowData.auditStatus === '检测退至金融机构' || this.rowData.auditStatus === '认证退至金融机构' || this.rowData.auditStatus === '协会退至金融机构') {
        flowTodo(this.$route.query.id).then(res => {
          ...
          rzWorkFlow(obj).then(res => {
            this.flowIdXH = res.flowID
            console.log(this.flowIdXH, 9999999)
            const obj0 = {
              name: res === null ? '' : res.name,
  ...
  methods: {
    ...
    getStatus(data) {
      statusComplate(this.riidNum).then(res => {
        if (res === '待提交检测' || res === '已认证待初审' || res === '待检测' || res === '已检测待初审' || res === '复审待审核') {
          this.loading.close()
          this.$message({
            message: '提交审核成功',
            type: 'success' })
    ...
    sub(formName) {
      ...
      const obj = {}
      if (this.$route.query.num === '2' && this.$route.query.update !== '重大变更' && this.rowData.appFilePath === '2') {
        obj.flowID = 'recordChange'
      } else if (this.$route.query.num === '2' && this.$route.query.update !== '重大变更' && this.rowData.appFilePath === '1') {
        obj.flowID = 'recordDetectionChange'
        ...
        obj.flowID = 'recordMajorChange'
      } else if (this.$route.query.auditStatus === '审核未通过' || this.$route.query.auditStatus === '审核不通过') {
        if (this.addVal === '其他') {
          obj.flowID = 'recordDetection'
        } else if (this.addVal === '金融科技产品认证') {
          obj.flowID = 'record'
        }
      } else if (this.$route.query.auditStatus !== '审核未通过' && this.$route.query.auditStatus !== '审核不通过') {
        if (this.$route.query.appFilePath && this.$route.query.appFilePath === '2') {
          obj.flowID = 'record'
        } else if (this.$route.query.appFilePath && this.$route.query.appFilePath === '1') {
          obj.flowID = 'recordDetection'
        }
      }
      if (this.rowData.auditStatus === '协会退至金融机构') {
        ...
        getNext(this.testData).then(res => {
          ...
          getUsers(this.nextNodeAll).then(res => {
            ...
            if (this.$route.query.update !== '更新') {
              ...
              } else if (this.form.detectionOrg) {
                this.optionsTest.forEach(item => {
                  if (item.riid === this.form.detectionOrg) {
                    this.workFlowParams.flowNextResource.nextNodes[0].authors.Author.selectAuthorName = item.rinm
                    this.workFlowParams.flowNextResource.nextNodes[0].authors.Author.selectAuthor = 'P_' + item.riid
        ...
        if (this.$route.query.auditStatus === '审核未通过') {
          if (this.addVal === '其他') {
    ...
    submit1() {
      ...
      this.paramsObj.appFilePath = this.addVal === '其他' ? '1' : '2'
      setTimeout(() => {
        if (this.remnant2 === 0) {
          delete this.paramsObj.workflow
          if (this.$route.query.num === '2' && this.$route.query.update !== '重大变更') {
            ...
            this.paramsObj.oldattachIds = this.oldattachIds.join(',')
            this.paramsObj.deleteattachIds = this.deleteattachIds
            this.paramsObj.flag = '4'// 跨级退回提交时flag是2，不跨级是2
            proChange(this.paramsObj, this.form.approveOrg).then(res => {
              this.getStatus('提交审核成功')
            }).catch(() => {
              this.loading.close()
            ...
            this.paramsObj.deleteattachIds = this.deleteattachIds
            if (this.paramsObj.appFilePath === '1') {
              this.paramsObj.flag = '2'// 跨级退回提交时flag是2，不跨级是2
              proChangeToTest(this.paramsObj, this.form.approveOrg).then(res => {
                this.getStatus('提交审核成功')
              }).catch(() => {
                this.loading.close()
              ...
              this.paramsObj.flag = '3'// 跨级退回提交时flag是2，不跨级是2
              proChange(this.paramsObj, this.form.approveOrg).then(res => {
                this.getStatus('提交审核成功')
              }).catch(() => {
                this.loading.close()

  ...
  <el-card class="AppRegistration AuthenticationTest">
    ...
    <el-form ref="form" :model="form" :rules="rules">
      <fieldset class="fieldsetStyle">
        <legend style="margin-left: 40px; font-size:17px">产品与服务信息填写</legend>
        ...
        <el-row :gutter="20" style="margin: 0 30px">
          <el-col :span="24">
            ...
            >
              ...
              <el-button type="primary" @click="addRequirement()">+ 新增</el-button>
        ...
        <el-row :gutter="20" style="margin: 0 30px">
          <el-col>
            <el-table
              v-if="tableDataSelect1.length>0"
    ...
    <div>
      <el-row style="text-align:center;">
        <el-col :span="24">
          <el-button type="primary" @click="submitWorkflow()">保存</el-button>
...
<script>
import { gist } from '@/api/dataDic.js'
import { getSuperviseTree } from '@/api/dataDic.js'
import { commonValidate2 } from '@/utils/formValidate'
import { add } from '@/api/TechnologyProducts.js'
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        productPeople: [{ required: true, message: '内容不能为空且最多200个字符', trigger: 'blur', max: 200 }],
        productTel: [{ required: true, message: '内容不能为空且最多200个字符', trigger: 'blur', max: 200 }]
      }
    }
  },
  computed: {
  ...
  methods: {
        ...
        gist(this.$refs.select.getCheckedNodes()[0].data.id).then(res => {
          this.tableDataSelect = res.map(item => {
            return {
              ...
              disable: false,
              id: item.id,
              gistNum: item.gistNum ? item.gistNum : '--'
            }
          })
          this.tableDataSelect.map(item => {
            this.tableDataSelected.map(itemed => {
              if (item.id === itemed.id) {
                item.disable = true
              }
            })
          })
    ...
    handleChange(value) {
      console.log(value, this.$refs.select.getCheckedNodes()[0])
    },
    addRequirement() {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$confirm('是否确定提交？', '提示', {
            ...
            if (this.tableDataSelect1.length > 0) {
              this.IsloadingA = this.$loading({
                lock: true
              })
              this.paramsObj.submiSsion = '0'
              this.paramsObj.superVersions = this.tableDataSelect1.map(item => {
                return {
              ...
              add(this.paramsObj).then(res => {
                this.$router.push({
                  name: 'myProducts'
                })
                this.IsloadingA.close()
                this.$message({
                  message: '提交成功',
                  type: 'success' })
              })

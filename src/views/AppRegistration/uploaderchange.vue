...
<template>
  <div v-loading.fullscreen.lock="uploadLoading" :element-loading-text="`文件正在上传，请勿离开！已上传： ${fileDataLen-isOver}/${fileDataLen}`" class="uploader-common">
    ...
    <div :before-upload="handleBefore" class="button-group clearfix">
      ...
      <el-button :id="selectFileButton" ref="file" :disabled="fileData.length>0" :class="selectFileButton" type="primary" plain style="display: inline-block">上传</el-button>
...
import { HXSearchList } from '@/api/appbeian/register.js'
...
export default {
  ...
  computed: {
    ...
    config() {
      ...
      return {
        ...
        uploadURL: 'nifa-autonomy-file/upload/upload' /** HTML5上传的URI */,
  ...
  mounted() {
    setTimeout(() => {
      this.config.postVarsPerFile.noteId = this.fileid
      // this.config.postVarsPerFile.extend1 = this.extend1
      this.vivwTable()
    }, 1000)
  ...
  methods: {
    vivwTable() {
      this.fileData = []
      HXSearchList({ noteId: this.fileid, ownSystem: '8' }).then(res => {
        res.forEach(element => {
    ...
    deleteFn(val) {
      this.$confirm('确定要删除？', '提示', {
        ...
        if (val.row.id === undefined) { // 回显的
          val.row.loading = false
          this.istrue = false
          this.$emit('delId', val)
          this.upload.cancel()
          this.fileData.splice(val.$index, 1)
        } else {
          this.upload.cancel()
          this.fileData.splice(val.$index, 1)
        }
